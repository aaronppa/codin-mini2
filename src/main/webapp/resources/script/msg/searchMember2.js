		...
		source: function(request, response){
			...
			.done(function(data){
				...
				response($.map(data,function(item){
					...
					console.log(found);
					console.log(found.indexOf($('#searchRecipient').val()));
					if(found.indexOf($('#searchRecipient').val()) >= 0){
						console.log("item.memberName: "+item.memberName);
						return{
							label : item.replace($('#searchRecipient').val(), "<span style='font-weight:bold;color:Blue;'>" + $( "#searchRecipient" ).val() + "</span>"),
